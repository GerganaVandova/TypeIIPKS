...
def main():
	...
	#print record.id
	if record.id == "AD000014.1_MSGY423_10429_11460_Mycobacterium_tuberculosis_sequence_from_clone_y423_0_1_42741_6e-10":
	    nrid = "AD000014.1_10429_11460"
	    break
	    seqid = record.id.split("__")[0]
            coord = record.id.split("__")[1]
            leftcoord = coord.split("_")[0]
            rightcoord = coord.split("_")[1]
            nrid = seqid + "_" + leftcoord + "-" + rightcoord
            nr_id_to_seq[nrid] = record.seq
            nonredundant_ids.append(nrid)
    handle.close()
    ...
    for line in f.readlines():
	    ...
	    if seq_id_coord == "AD000014.1_MSGY423_10429_11460_Mycobacterium_tuberculosis_sequence_from_clone_y423_0_1_42741_6e-10":
                seq_id_coord = "AD000014.1_10429_11460"
                break
                seqid = seq_id_coord.split("__")[0]
            	coord = seq_id_coord.split("__")[1]
            	leftcoord = coord.split("_")[0]
            	rightcoord = coord.split("_")[1]
            	seq_id_coord = seqid + "_" + leftcoord + "-" + rightcoord
            	print seq_id_coord
    ...
    print missing_ref, len(missing_ref), 78 - len(missing_ref)
    print count
